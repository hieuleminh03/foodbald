...
class Cooldown {
	...
	// Backward compatibility alias (returns frames)
	public inline function get(k) : Float   return getF(k);
	...
	// Backward compatibility alias (duration in frames)
	public inline function set(k:String, frames:Float, ?allowLower=true, ?onComplete:Void->Void) {
		setF(k, frames, allowLower, onComplete);
	}
	...
	// Backward compatibility alias (returns frames)
	public inline function hasSet(k, frames:Float)     return hasSetF(k, frames);

	public function update(?dt:Float=1.0) {
		var i = 0;
